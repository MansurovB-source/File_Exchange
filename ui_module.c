...
void render_download_window(struct ui_data *data, int8_t do_clear) {
    int rows, cols;
    ...
    for (int32_t i = 0; i < available_rows; i++) {
        struct list *action = get(data->ctx->events_module->download_list, i);
        if (action) {
            struct transfer_progress *progress = action->value;
...
void render_upload_window(struct ui_data *data, int8_t do_clear) {
    int rows, cols;
    ...
    int32_t transfer_start = 1;
    for (int32_t i = 0; i < available_rows; i++) {
        struct list *action = get(data->ctx->events_module->upload_list, i);
        if (action) {
            struct transfer_progress *progress = action->value;
            size_t name_len = strlen(progress->triplet.filename);
...
void render_events_log(struct ui_data *data, int8_t do_clear) {
    ...
    int32_t log_start = 2;
    for (int32_t i = 0; i < available_rows; i++) {
        struct list *action = get(data->ctx->events_module->actions_list, i);
        if (action) {
            wmove(data->events_win, log_start + i, 1);
            wprintw(data->events_win, action->value);
        } else {
            break;
        }
    }
...
void render_all(struct ui_data *ui_data) {
